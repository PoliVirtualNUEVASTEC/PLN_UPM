...
- [x] 0.1 Frontera de escritura de PR1: SOLO `Data/Dialogue/` y este directorio de cambio
  (`openspec/changes/2026-09-09-m6-generador-markov/`). Nada en `Runtime/` todavía.
- [x] 0.2 Frontera de escritura de PR2: `Runtime/Dialogue/MarkovChainBuilder.cs`,
  ...
  referencia).
- [x] 0.3 Cero cambio en `Runtime/Core/Ports.cs`, `Dtos.cs`, `Enums.cs`, `PersonalityId.cs`.
...
- [x] 0.4 `Tests/EditMode/Core/DialogueGeneratorContract.cs` no se modifica en ningún PR:
  `MarkovDialogueGeneratorTests` hereda, no edita, la base.
- [x] 0.5 Este módulo es IA (M6): `/sdd-ff` no aplica — cada PR pasa por el flujo completo
  propuesta → diseño → tareas → revisión humana, sin atajos.
- [x] 0.6 No confundir `Data/Dialogue/` (lo que dice el NPC, nuevo en este cambio) con
...
- [x] 1.1 Definir el esquema JSON de `Data/Dialogue/<personalidad>.json`: un objeto con una
...
- [x] 1.2 Crear `Data/Dialogue/README.md`: explica el propósito del corpus (semilla para las
  cadenas de Markov de M6), el esquema, y la diferencia explícita con `Data/Corpus/` (M3, lo que
  dice el *usuario*, no el NPC).
- [x] 1.3 Redactar `Data/Dialogue/grosero.json`, `histerico.json`, `introvertido.json`,
...
- [x] 2.1 RED: `Tests/EditMode/Dialogue/MarkovChainBuilderTests.cs` — con una lista vacía,
...
- [x] 2.2 GREEN: Crear `Runtime/Dialogue/MarkovChainBuilder.cs` (tabla de bigramas, paseo
...
- [x] 2.3 RED: `Tests/EditMode/Dialogue/MarkovDialogueGeneratorTests.cs :
  DialogueGeneratorContract` — `CreateSubject()` carga el corpus semilla real de
  `Data/Dialogue/`.
- [x] 2.4 GREEN: Crear `Runtime/Dialogue/MarkovDialogueGenerator.cs` implementando
...
- [~] 2.5 Prueba manual dedicada (no NUnit, script o botón de Editor): para las 4 personalidades
  ...
  > Cubierto en espíritu por `MarkovDialogueGeneratorTests.Ninguna_personalidad_ni_estado_devuelve_texto_vacio_en_volumen`
  > (4 × 3 × 250 = 3000 llamadas dentro de NUnit). Falta, si el equipo lo quiere, el script/botón de Editor separado.
- [x] 2.6 Confirmar que ningún otro punto del código pasa a instanciar
...
- [x] 3.1 Crear `openspec/specs/generador-dialogo-m6/spec.md` — primera spec formal de M6,
  > Redactada como delta en `changes/2026-09-09-m6-generador-markov/specs/generador-dialogo-m6/spec.md`;
  > `sdd-archive` la promueve a `openspec/specs/` al cerrar (mismo flujo que M0/M1/M4/M5).
  ...
  el esquema de M4).
- [x] 3.2 Actualizar la sección M6 de `Docs/MODULES.md`: reemplazar "solo doble" por el estado
  real (generador por cadenas de Markov, tamaño del corpus semilla usado, referencia a la nueva
  spec).
- [x] 3.3 Revisar que el diff acumulado de PR1+PR2+PR3 respeta exactamente la lista de
