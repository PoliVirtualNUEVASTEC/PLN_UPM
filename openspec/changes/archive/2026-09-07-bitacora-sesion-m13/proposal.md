...
|---|---|---|
| `sqlite-net-pcl` no es en realidad un paquete UPM resoluble desde `package.json`; el AD original lo asumió mal | Materializado | Resuelto (2026-09-08): vendorizado manual de los DLL administrados y los binarios nativos en `Runtime/SessionLog/Sqlite/`, mismo patrón que M1 con Vosk. Falta validar en dispositivo/IL2CPP (tarea 2.5) |
| `sqlite-net-pcl` 1.9.172 depende por defecto de `SQLitePCLRaw.bundle_green` 2.1.2, cuyo binario nativo tiene una vulnerabilidad de severidad alta (CVE-2025-6965, corrupción de memoria en SQLite < 3.50.2) | Materializado, mitigado | Se forzaron los paquetes `SQLitePCLRaw.lib.e_sqlite3`/`.lib.e_sqlite3.android` a la versión 2.1.13 (verificada binariamente: embebe SQLite 3.53.3, por encima del umbral seguro) al vendorizar, en vez de usar los binarios que trae por defecto `bundle_green` |
