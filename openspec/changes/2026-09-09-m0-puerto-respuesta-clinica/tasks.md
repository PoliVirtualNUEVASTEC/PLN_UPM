...
- [ ] 0.1 **Este es un cambio de contrato (M0).** Autoriza tocar `Runtime/Core/` y
...
- [ ] 0.2 Frontera de escritura: `Runtime/Core/ClinicalCaseId.cs` (nuevo),
...
- [ ] 0.3 **Cambio puramente aditivo.** NO renombrar, reordenar, ni cambiar el valor de
...
- [ ] 0.4 `ContractTypeTests` se **amplía** (casos nuevos para los tipos nuevos), no se
  reescribe: ningún caso existente cambia. `ContractVersionChangelogTests` y
  `CoreAssemblyPurityTests` no se tocan.
- [ ] 0.5 Este módulo es M0: `/sdd-ff` no aplica. El flujo completo propuesta → diseño →
  tareas → revisión humana + co-revisión de M0.
- [ ] 0.6 `NpcAi.Core` compila con `noEngineReferences: true`. Los 3 tipos nuevos son C#
  puro: `string`, `bool`, `struct`, y tipos de v1. Cero `using UnityEngine`.

## Phase 1: Tipos, puerto y versión

- [ ] 1.1 Crear `Runtime/Core/ClinicalCaseId.cs`: copiar `Runtime/Core/PersonalityId.cs`
...
- [ ] 1.2 En `Runtime/Core/Dtos.cs`, agregar `readonly struct ClinicalResponse` con
...
- [ ] 1.3 En `Runtime/Core/Ports.cs`, agregar `interface IClinicalResponder` (ver
...
- [ ] 1.4 En `Runtime/Core/Contract.cs`, subir `Version` a `2`. **En el mismo commit**,
...
- [ ] 1.5 En `Docs/CONTRACT-CHANGELOG.md`, corregir la línea de regla de integración: de
...
- [ ] 2.1 RED/estructura: crear `Tests/EditMode/Core/ClinicalResponderContract.cs`,
...
- [ ] 2.2 Escribir los `[Test]` no abstractos (nombres en español, estilo del repo):
...
- [ ] 2.3 Confirmar que `ClinicalResponderContract` compila y **pasa contra
...
- [ ] 2.4 Ampliar `Tests/EditMode/Core/ContractTypeTests.cs` con los casos de la tarea del
